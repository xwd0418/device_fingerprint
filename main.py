...
class Objective:
    ...
    def __call__(self, trial: optuna.trial.Trial) -> float:
        ...
        max_epoch = 44  if len(sys.argv) > 2 else config['experiment']['num_epochs']
...
if __name__ == "__main__":
    ...
    if should_early_stop is None:
        should_early_stop = "resnet" in exp_name 
    ...
    n_trials = config.get('n_trials') if config.get('n_trials') else 20
    study.optimize(Objective(should_early_stop),
                   n_trials=n_trials, 
                #    callbacks=[delete_bad_ckpt_callback]
                   )
