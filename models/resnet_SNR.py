...
class UpBlock(nn.Module):
    def __init__(self, inplanes, planes, upsample=False):
        ...
        self.conv = nn.Conv1d(inplanes, planes, 1, 1)
...
class Conv1x1nonLinear(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        self.conv = nn.Conv1d(in_channels, out_channels, 1, stride=stride, padding=0, bias=False)
...
class ResNet_SNR_Causality(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            return x_4, end_points, \
                   F.softmax(self.fc1(self.global_avgpool(x_IN_1).view(x_IN_1.size(0), -1)) , dim=1), \
                   F.softmax(self.fc1(self.global_avgpool(x_1).view(x_1.size(0), -1)) , dim=1), \
                   F.softmax(self.fc1(self.global_avgpool(x_1_useless).view(x_1_useless.size(0), -1)) , dim=1), \
                   F.softmax(self.fc2(self.global_avgpool(x_IN_2).view(x_IN_2.size(0), -1)) , dim=1), \
                   F.softmax(self.fc2(self.global_avgpool(x_2).view(x_2.size(0), -1)) , dim=1), \
                   F.softmax(self.fc2(self.global_avgpool(x_2_useless).view(x_2_useless.size(0), -1)) , dim=1), \
                   F.softmax(self.fc3(self.global_avgpool(x_IN_3).view(x_IN_3.size(0), -1)) , dim=1), \
                   F.softmax(self.fc3(self.global_avgpool(x_3).view(x_3.size(0), -1)) , dim=1), \
                   F.softmax(self.fc3(self.global_avgpool(x_3_useless).view(x_3_useless.size(0), -1)) , dim=1), \
